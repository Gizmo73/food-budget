...
export function newProduct(name, store, fields = {}) {
  return {
    ...
    portionGrams: 0,
    barcodes: [],
    // packs of this exact product put on the shopping list by hand, so one
    // ingredient can carry two different kinds at once
    extraPacks: 0,
    offer: null,
    priceUpdated: "",
    /* When this figure was last actually counted, as opposed to worked out
       from a receipt. Stock only ever went up on its own, so what matters is
       not the number but whether anybody has looked in the cupboard since. */
    stockCheckedAt: "",
...
export function moveProduct(db, fromId, productId, toId) {
  ...
  for (const meal of db.meals || []) {
    ...
    // a rewritten meal is an edit, and has to outrank the other phone's copy
    if (changed) meal.updatedAt = stamp;
  }

  from.updatedAt = stamp;
  to.updatedAt = stamp;

  /* It has left this ingredient, and the other phone still has it here. Say
     so, or the next merge hands it back and its stock is counted twice. */
  markDeleted(db, "prod", fromId, productId);
...
export function copyToShop(product, taken = []) {
  const made = {
    ...
    stockPortions: 0,
    // a request to buy the original is not a request to buy this fresh copy
    extraPacks: 0,
...
export function seed() {
  ...
  // never edited, so an edit on any device outranks them
  const meals = [
    { id: "pie-and-mash", name: "Pie and Mash", updatedAt: "", items: [
    ...
    { id: "spaghetti-bolognese", name: "Spaghetti Bolognese", updatedAt: "", items: [
    ...
    { id: "chicken-stir-fry", name: "Chicken Stir Fry", updatedAt: "", items: [
  ...
  return {
    ...
    planUpdatedAt: "",
    deleted: {},
    /* This is the demo list every new install opens on, not anybody's work.
       The flag is cleared by the first edit, and it is what lets a merge tell
       "a list of their own" from "they have not started yet". */
    demo: true,
  };
}
  ...
  const named = (list, fallbackName) =>
    list.map((p) => {
      ...
      return {
        ...
        // hand-added packs of this exact product, kept so they survive a reload
        extraPacks: Math.max(0, Number(p.extraPacks) || 0),
        offer: cleanOffer(p.offer),
        priceUpdated: p.priceUpdated || "",
        // older data has never been counted, which is exactly what it should say
        stockCheckedAt: typeof p.stockCheckedAt === "string" ? p.stockCheckedAt : "",
...
/* A deletion is a fact that has to travel, and the only way to send one
   through a file that holds what exists is to also record what no longer
   does. The key says what kind of thing and which one; the value is when. */
export const graveKey = (kind, ...ids) => [kind, ...ids].join(":");

export function markDeleted(db, kind, ...ids) {
  const when = new Date().toISOString();
  db.deleted = { ...(db.deleted || {}), [graveKey(kind, ...ids)]: when };
  return db;
}

/* A record survives its own headstone if it was touched afterwards: you
   edited the thing they had already binned, and acting last wins. An empty
   stamp on the record means it has never been touched, so the deletion wins. */
const buried = (deleted, key, touchedAt) => {
  const when = deleted[key];
  return !!when && !((touchedAt || "") > when);
};

function cleanDeleted(raw) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return {};
  const out = {};
  for (const [key, when] of Object.entries(raw)) {
    if (typeof key === "string" && typeof when === "string" && when) out[key] = when;
  }
  return out;
}

/* Headstones are kept long enough that no phone can have been away longer,
   and no longer, or the file grows for ever. */
const KEEP_GRAVES_DAYS = 120;

function pruneDeleted(deleted) {
  const cutoff = new Date(Date.now() - KEEP_GRAVES_DAYS * 86400000).toISOString();
  const out = {};
  for (const [key, when] of Object.entries(deleted)) if (when > cutoff) out[key] = when;
  return out;
}

/* Hand-written lines on the shopping list: things with no item behind them,
   typed straight onto a shop while you are standing in it. They carry no
   price and never touch stock, so a shop and the words are all there is. The
   stamp is what a merge compares, and what a headstone is measured against. */
const cleanJottings = (raw) =>
  Array.isArray(raw)
    ? raw
...
export function migrate(db) {
  ...
  /* A single planned line, as an item under a meal or a loose edit or extra.
     Portions describe one person from v6 on, so nothing is halved here: loose
     edits and extras did not exist before then. */
  const cleanPlanItem = (it) => ({
    ingredientId: it.ingredientId,
    productId: it.productId || "",
    portions: Math.max(0, Number(it.portions) || 0),
    by: it.by === "grams" ? "grams" : "portions",
    grams: Math.max(0, Number(it.grams) || 0),
  });
  const cleanItems = (list) =>
    Array.isArray(list) ? list.filter((it) => it && it.ingredientId).map(cleanPlanItem) : [];

  /* One cell's loose override: the items that stand in for the base meal on
     this day, plus a display name. Null when the cell was never edited. */
  const cleanOverride = (o) =>
    o && typeof o === "object" && Array.isArray(o.items)
      ? { name: typeof o.name === "string" ? o.name : "", items: cleanItems(o.items) }
      : null;
  const cleanEdits = (e) => {
    if (!e || typeof e !== "object") return null;
    const out = {};
    for (const { key } of SLOTS) {
      const cell = Array.isArray(e[key]) ? e[key] : null;
      if (!cell) continue;
      const o0 = cleanOverride(cell[0]);
      const o1 = cleanOverride(cell[1]);
      if (o0 || o1) out[key] = [o0, o1];
    }
    return Object.keys(out).length ? out : null;
  };
  const cleanExtras = (x) => {
    if (!Array.isArray(x)) return null;
    const p0 = cleanItems(x[0]);
    const p1 = cleanItems(x[1]);
    return p0.length || p1.length ? [p0, p1] : null;
  };
  const cleanWritten = (w) => {
    if (!Array.isArray(w)) return null;
    const out = w.map((t) => String(t || "").trim()).filter(Boolean);
    return out.length ? out : null;
  };

  const plan = Array.from({ length: 14 }, (_, i) => {
    const was = Array.isArray(db.plan) ? db.plan[i] : null;
    const obj = was && typeof was === "object" && !Array.isArray(was);
    const day = obj
      ? { breakfast: pair(was.breakfast), lunch: pair(was.lunch), dinner: pair(was.dinner) }
      : { breakfast: pair(null), lunch: pair(null), dinner: pair(typeof was === "string" ? was : null) };
    // on-the-fly edits and extras ride along only when a day actually has them,
    // so an untouched plan stays as plain as it was
    const edits = obj ? cleanEdits(was.edits) : null;
    if (edits) day.edits = edits;
    const extras = obj ? cleanExtras(was.extras) : null;
    if (extras) day.extras = extras;
    // a takeaway or a dinner out, written straight onto the day with no
    // ingredients behind it - same "ride along only if present" rule
    const written = obj ? cleanWritten(was.written) : null;
    if (written) day.written = written;
    return day;
  });
  ...
  return {
    ...
    // only the untouched starting list carries this, and only until it is edited
    demo: db.demo === true,
    plan,
    /* The plan has its own clock, separate from the database's. Merging used
       to take the plan from whichever whole snapshot was saved last, so a
       price typed on one phone was enough to make its untouched plan outrank
       the fortnight the other person had just filled in. */
    planUpdatedAt: typeof db.planUpdatedAt === "string" ? db.planUpdatedAt : db.updatedAt || "",
    ...
    /* What has been deleted, and when. A merge is a union, so without this it
       cannot tell "they added it" from "I threw it out", and anything either
       of you deleted came back on the next pull. */
    deleted: cleanDeleted(db.deleted),
    jottings: cleanJottings(db.jottings),
    meals: (db.meals || []).map((m) => ({
      id: m.id || uid(),
      name: m.name || "Meal",
      // stamped on every edit, so two copies of one meal can be compared
...
/* Whole days between two dates. Both are read at midday, so a clock change
   cannot make a day 23 or 25 hours long and round to the wrong number. */
export function daysBetween(from, to) {
  const at = (d) => new Date(`${String(d || "").slice(0, 10)}T12:00:00`).getTime();
  const a = at(from);
  const b = at(to);
...
const emptyDay = () => ({ breakfast: [null, null], lunch: [null, null], dinner: [null, null] });

const plannedIn = (day) =>
  SLOTS.reduce((n, s) => n + ((day && day[s.key]) || []).filter(Boolean).length, 0);

/* Slide the plan so its meals keep the dates they were chosen for.

   A day of the plan is stored by its position, and the start date is what
   turns position into a date. So moving the start moved every meal with it: a
   fortnight beginning on Monday with something planned for Wednesday, started
   a day later instead, quietly served that meal on Thursday. Moving the start
   forward a day slides the plan back a day, which leaves it where you put it.

   Days pushed outside the fortnight are gone, and counted, because a plan that
   silently drops the meals you had chosen is worse than one that says so. */
export function shiftPlan(plan, days) {
  if (!Array.isArray(plan) || !days) return { plan, lost: 0 };
  const out = Array.from({ length: plan.length }, emptyDay);
  let lost = 0;
  plan.forEach((day, i) => {
    const to = i - days;
    if (to >= 0 && to < plan.length) out[to] = day;
    else lost += plannedIn(day);
  });
  return { plan: out, lost };
}
...
function mergeProducts(mine, theirs, deleted, ingId) {
    ...
    const label =
      fed && theirFed
        ? (t.nutritionUpdated || "") > (m.nutritionUpdated || "")
          ...
          : m
        : fed || theirFed || m;
    /* Stock is the higher of the two, because a pack somebody bought is a
       physical fact, unless one of you has actually counted it since. A count
       is somebody looking in the cupboard, which beats arithmetic. */
    const mineCount = m.stockCheckedAt || "";
    const theirCount = t.stockCheckedAt || "";
    const counted = mineCount || theirCount ? (theirCount > mineCount ? t : m) : null;

    byId.set(t.id, {
      ...winner,
      stockPortions: counted
        ? Number(counted.stockPortions) || 0
        : Math.max(Number(m.stockPortions) || 0, Number(t.stockPortions) || 0),
      stockCheckedAt: theirCount > mineCount ? theirCount : mineCount,
      // a barcode either of you scanned is worth keeping
      barcodes: [...new Set([...(m.barcodes || []), ...(t.barcodes || [])])],
      // hand-added packs are a physical request, so keep the higher count, the
      // same way the ingredient's own loose count merges
  ...
  /* One you stopped buying, or moved to another ingredient, must not come
     back from the other phone's copy carrying its stock with it. A price
     dated after the deletion is a real reason to keep it. */
  const kept = [...byId.values()].filter(
    (p) => !buried(deleted, graveKey("prod", ingId, p.id), p.priceUpdated)
  );
  // never leave an ingredient with nothing to buy: it could not be priced
  return kept.length ? kept : [...byId.values()];
}

export function mergeSnapshots(local, remote) {
  const mine = migrate(local);
  const theirs = migrate(remote);
  const notes = { added: 0, updated: 0, kept: 0, removed: 0, planFrom: "local" };

  /* A phone still showing the starting demo list has nothing to contribute,
     and merging it in is how eleven demo items and three demo meals arrived
     on the other person's list the moment somebody accepted an invite. Taking
     the shared list whole is what joining is supposed to mean. */
  if (mine.demo && !theirs.demo) {
    return {
      db: { ...theirs, demo: false, updatedAt: new Date().toISOString() },
      notes: { ...notes, added: theirs.ingredients.length, planFrom: "remote" },
    };
  }
  if (theirs.demo && !mine.demo) return { db: { ...mine, updatedAt: new Date().toISOString() }, notes };

  /* Both sides' headstones, the later one winning where both have buried the
     same thing, so a deletion reaches the other phone instead of being
     resurrected by the union below. */
  const deleted = { ...mine.deleted };
  for (const [key, when] of Object.entries(theirs.deleted)) {
    if (!deleted[key] || when > deleted[key]) deleted[key] = when;
  }
  ...
  for (const t of theirs.ingredients) {
    ...
    byId.set(t.id, {
      ...winner,
      products: mergeProducts(m.products, t.products, deleted, t.id),
  ...
  /* Meals merge the same way items do: the union, and where both hold the
     same meal the later edit wins. Keeping the local copy unconditionally,
     as this used to, meant a meal you changed never reached the other
     person — they saw new meals appear and edits to old ones never arrive. */
  const mealsById = new Map(mine.meals.map((m) => [m.id, m]));
  for (const t of theirs.meals) {
    const m = mealsById.get(t.id);
    if (!m) {
      mealsById.set(t.id, t);
      notes.added += 1;
      continue;
    }
    if ((t.updatedAt || "") > (m.updatedAt || "")) {
      mealsById.set(t.id, t);
      notes.updated += 1;
    }
  }

  /* The plan travels on its own stamp, so pricing something on one phone no
     longer outranks a fortnight planned on the other. Who the two people are
     and when the fortnight starts belong to the plan and go with it. */
  const planNewer = (theirs.planUpdatedAt || "") > (mine.planUpdatedAt || "");
  if (planNewer) notes.planFrom = "remote";
  const remoteNewer = (theirs.updatedAt || "") > (mine.updatedAt || "");

  const ingredients = [...byId.values()].filter((i) => {
    if (!buried(deleted, graveKey("ing", i.id), i.updatedAt)) return true;
    notes.removed += 1;
    return false;
  });

  const meals = [...mealsById.values()].filter((m) => {
    if (!buried(deleted, graveKey("meal", m.id), m.updatedAt)) return true;
    notes.removed += 1;
    return false;
  });

  /* Hand-written lines are a union like everything else, the later wording
     winning where both phones hold the same one. The headstone matters more
     here than anywhere: striking one off in the shop is a deletion, and
     without it the other phone would write it straight back on the next
     merge — the one thing that would make the list untrustworthy mid-shop. */
  const jotById = new Map(mine.jottings.map((j) => [j.id, j]));
  for (const t of theirs.jottings) {
    const m = jotById.get(t.id);
    if (!m || (t.at || "") > (m.at || "")) jotById.set(t.id, t);
  }
  const jottings = [...jotById.values()].filter(
    (j) => !buried(deleted, graveKey("jot", j.id), j.at)
  );

  // a meal that has gone cannot stay on the plan pointing at nothing
  const alive = new Set(meals.map((m) => m.id));
  const plan = (planNewer ? theirs.plan : mine.plan).map((day) => {
    const out = {
      breakfast: day.breakfast.map((id) => (id && alive.has(id) ? id : null)),
      lunch: day.lunch.map((id) => (id && alive.has(id) ? id : null)),
      dinner: day.dinner.map((id) => (id && alive.has(id) ? id : null)),
    };
    // the plan is taken whole from one side, so its loose edits, extras and
    // anything written in come with it — they were normalised by migrate() above
    if (day.edits) out.edits = day.edits;
    if (day.extras) out.extras = day.extras;
    if (day.written) out.written = day.written;
    return out;
  });
  ...
  return {
    db: {
      ...
      demo: false,
      plan,
      planUpdatedAt: planNewer ? theirs.planUpdatedAt : mine.planUpdatedAt,
      people: planNewer ? theirs.people : mine.people,
      planStart: planNewer ? theirs.planStart : mine.planStart,
...
export const DEFAULT_SETTINGS = {
  ...
  lastPull: "",
  lastPush: "",
  /* The git sha of the shared file as this device last saw it. Whether the
     file has moved on is a question about the file, not about two phones'
     clocks, and this is the only answer that does not depend on them agreeing. */
  lastSha: "",
  collapsedItems: [],
  ...
  mealFilter: "all",
  /* Which parts of a product editor are unfolded, by kind rather than by
     product: closing nutrition should close it on all forty items, not one. */
  openSections: ["portion"],
  collapsedList: [],
  person: "",
  showRepo: false,
  theme: "system",
  /* Settings belong to the device, not to the shared database: a phone's
     theme has no business travelling to somebody else's phone. The accent
     is the same kind of thing, so two people sharing one list can each have
     their own colour. Empty means the app's own yellow. */
  accent: "",
  autoMerge: true,
  warnOnLeave: true,
};
