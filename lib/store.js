...
export function newIngredient(store) {
  return {
    ...
    id: uid(), name: "New item", portionsPerPack: 1, pricePerPack: 0, stockPacks: 0,
...
export function mergeSnapshots(local, remote) {
  ...
  for (const t of theirs.ingredients) {
    ...
    byId.set(t.id, {
      ...
      stockPacks: Math.max(Number(m.stockPacks) || 0, Number(t.stockPacks) || 0),
